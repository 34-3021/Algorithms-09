...
std::unordered_map<hb, segment_entry> substrings;
...
int main() {
    ...
    for (int i = 0; i < reference_len; i++) {
        for (int j = i; j < reference_len; j++) {
            ...
            if (!substrings.contains(h1)) {
                substrings[h1] = {i, false};
            }
            if (!substrings.contains(h2)) {
                substrings[h2] = {i, true};
            }
        }
    }
    ...
    for (int i = query_len - 1; i >= 0; i--) {
        for (int j = i; j < query_len; j++) {
            ...
            if (substrings.contains(h)) {
                int jump_from = j + 1;
                if (f[i] > 1 + f[jump_from]) {
                    f[i] = 1 + f[jump_from];
                    int begin = substrings[h].begin;
                    bool inverted = substrings[h].inverted;
